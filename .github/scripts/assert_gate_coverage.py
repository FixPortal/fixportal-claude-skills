...
# fail-open on ordinary YAML. (item 6 of the follow-up batch)
...
def step_lines(block, run_index, key_indent):
    ...
    exist at that spelling was being required HIGH. (item 3 of the follow-up batch)
...
def gated_run_bodies(lines, jobs, needs, gate_job, root):
    ...
    while pending:
        ...
        # below; a sibling step's is not (item 3 of the follow-up batch). Computed
...
# (Review follow-up on the follow-up batch.)
...
def check_file(workflow_path, gate_job, exempt, conditional_exempt, *, on_empty="fail"):
    ...
    # (item 4 of the follow-up batch). And block-scalar `run:` payloads are skipped:
