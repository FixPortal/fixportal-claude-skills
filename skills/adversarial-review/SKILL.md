...
The pipeline has three phases, and the invariants matter more than the tools:
...
- **Reviewer X** — the Copilot wrapper. Write the Phase 1 brief to a file in
  ...
  `pwsh -NoProfile -File ~/.claude\skills\adversarial-review\external-review.ps1 -Instruction (Get-Content "<workdir>\phase1-brief.txt" -Raw) -DiffPath "<workdir>\review-diff.txt"`
  The script pins `gpt-5.4`. Reviewer X works from the diff only.
...
- Reviewer X — `external-review.ps1` with the Phase 2 brief and **both**
  files, the pooled findings passed as `-FindingsPath`:
  `pwsh -NoProfile -File ~/.claude\skills\adversarial-review\external-review.ps1 -Instruction (Get-Content "<workdir>\phase2-brief.txt" -Raw) -DiffPath "<workdir>\review-diff.txt" -FindingsPath "<workdir>\pooled-findings.txt"`
...
Present the judge's report directly — a severity-ranked finding list, each
tagged `[unanimous]` / `[majority]` / `[contested]`, contested items showing
both sides. Note if the cross-vendor reviewer was unavailable. Lead with the
report; do not narrate the phases you ran. Mention where the report was saved —
both the temporary working directory and the durable Obsidian vault copy (§6).

For a multi-chunk audit, do **not** answer after each chunk — run every chunk,
then synthesise (§5) and present the one consolidated report.

**Flag the contested findings as the action list.** A `[contested]` finding is
one the panel could not resolve from the diff alone — typically a real defect
*if* some path is exercised, where the reviewers split on whether it is. These
are exactly the findings worth verifying against the live code (run the code
path, check the caller, write the failing test) rather than acting on or
dismissing blind. Close the answer by naming the contested findings as the
recommended next step, and offer to verify them. Do not silently treat a
contested finding as resolved.
...
brief (below). Write its output to `report.md` and present that as the final
answer — a single ranked report for the whole audited surface, noting which
chunk(s) each finding came from. This is the artefact the run exists to
produce.
...
one-or-two-sentence "headline highs" note, the `working/` pointer, and — when
remediation followed — a `## Remediation (post-audit)` section. Template:
...
the `-Instruction` argument to `external-review.ps1`).
...
1. Verdict on every finding. For each F#, output one line:
   ...
   finding you genuinely cannot fault, you AGREE with.

2. Gaps. List any substantive defect the pooled set missed entirely, as full
   finding blocks in the Phase 1 format.

Work only from the diff and the pooled findings.
```
...
worth more to the user than a smoothed-over consensus. Be concise.
```
...
Be concise. Do not re-review the code — work only from the chunk reports.
```
...
subagent calls and 12 Copilot *calls*. State the projected cost when presenting
the chunk plan (§0a), and mention it too if the skill is being run repeatedly
in quick succession.
