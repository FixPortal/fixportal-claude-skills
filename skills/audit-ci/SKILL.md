...
| **Third-party SHA pins** | third-party actions pinned to full commit SHA, not a floating tag; and the SHA is current — cross-check the live upstream tag, dereferencing an annotated tag to its commit (`git ls-remote origin 'refs/tags/<tag>^{}'`, since `git/ref/tags/<tag>`'s `.object.sha` is the tag object, not the commit), and do not assume `scaffold-ci`'s documented SHA is fresh | drift: `raven-actions/actionlint@v2` tag (semgrep hook flags it); drift: SHA under `# v2` that no longer matches the live `v2` tag |
